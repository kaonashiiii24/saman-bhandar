...
export default function Footer() {
  ...
  const copyright = cms?.footer?.copyright_text || '© 2026 SamanBhandar Pvt. Ltd. All rights reserved.'
  const tagline = cms?.footer?.tagline || 'Made with care in Nepal.'

  const getLinks = (key, defaults) => {
    const val = cms?.footer?.[key];
  ...
  const platformLinks = getLinks('platform_links', [
  ...
  const companyLinks = getLinks('company_links', [
    { label: 'About Us', url: '/about' },
    { label: 'Contact', url: '/contact' }
  ]);

  const legalLinks = getLinks('legal_links', [

    ...
    <div className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-md">
      <div className="flex items-center justify-between h-16 px-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={onSave}
            className="inline-flex items-center gap-1.5 bg-gray-900 hover:bg-gray-800 text-white text-xs font-semibold px-4 py-2.5 rounded-lg transition-colors"
          >
            <Save size={14} /> Save
          </button>
          <button
            onClick={onPreview}
            className="inline-flex items-center gap-1.5 border border-gray-200 bg-white hover:bg-gray-50 text-gray-700 text-xs font-semibold px-3 py-2.5 rounded-lg transition-colors"
          >
            <Eye size={14} /> Preview
          </button>
          <div className="relative">
            <button
              ...
              className="inline-flex items-center justify-center w-9 h-9 border border-gray-200 bg-white hover:bg-gray-50 text-gray-700 rounded-lg transition-colors"
            >
            ...
            {showMore && (
              <div className="absolute right-0 mt-1.5 w-44 bg-white border border-gray-200 rounded-xl shadow-lg p-1.5 z-50 animate-scale-in">
                ...
                <button
                  onClick={() => { onRedo?.(); setShowMore(false) }}
                  disabled={!canRedo}
                  className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-100 w-full disabled:opacity-40"
                >
                  <Redo size={14} /> Redo
                ...
                <button
                  onClick={() => { onReset?.(); setShowMore(false) }}

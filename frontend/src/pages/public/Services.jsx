...
const defaultWhyStats = [
  { stat_key: 'total_sellers', label: 'Active sellers', sub: 'across Nepal' },
  { stat_key: 'total_hosts', label: 'Verified hosts', sub: 'manually checked' },
  { stat_key: 'total_cities', label: 'Locations covered', sub: 'and growing' },
  { stat_key: 'rating', label: 'Average rating', sub: 'from reviews', static_value: '4.8★' },
]
...
export default function Services() {
  ...
  const page = cms?.servicesPage || {}
  const allServices = cms?.services || []
  const services = allServices.slice(0, visibleServices)
  const hasMoreServices = allServices.length > visibleServices
  const whyStats = (cms?.servicesWhyStats && cms.servicesWhyStats.length > 0)
    ? cms.servicesWhyStats
    : defaultWhyStats
  const pricingPlans = cms?.pricingPlans || []
  const roleSteps = cms?.roleSteps || []

  const loadMoreServices = () => setVisibleServices(prev => prev + 6)

  const WHY = whyStats.map(stat => ({
    ...stat,
    num: stat.static_value || (
      stat.stat_key === 'total_sellers' ? `${publicStats?.total_sellers || 0}+` :
      stat.stat_key === 'total_hosts' ? `${publicStats?.total_hosts || 0}+` :
      stat.stat_key === 'total_cities' ? `${publicStats?.total_cities || 0}+` :
      '4.8★'
    )
  }))
    ...
    <div className="bg-[#FAFAF9] overflow-x-hidden">
      <section className="relative min-h-screen flex items-center overflow-hidden">
        <div 
          ...
          style={{ 
            backgroundImage: `url('${page.hero_image || '/images/services-hero.jpg'}')`,
        ...
        <div className="max-w-container mx-auto px-4 sm:px-6 lg:px-8 py-20 sm:py-28 w-full relative z-10">
          <div className="max-w-2xl">
            ...
            <h1 className={`font-display font-black text-5xl sm:text-6xl lg:text-7xl text-white tracking-tight leading-[1.08] mb-5 transition-all duration-700 delay-300 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
              {page.hero_title || 'Everything you need to store and move goods in Nepal.'}
            </h1>
            <p className={`text-white/50 text-lg leading-relaxed mb-8 max-w-xl transition-all duration-700 delay-500 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
              {page.hero_description || 'SamanBhandar connects renters, hosts and couriers on one platform — with tools built specifically for each role.'}
            </p>
            <div className={`flex flex-wrap gap-3 transition-all duration-700 delay-700 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-6'}`}>
              <Link to={page.hero_cta_primary_link || '/register'} className="inline-flex items-center gap-2 bg-brick hover:bg-brick-dark text-white font-display font-bold px-5 py-3 rounded-md transition-all hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 text-sm group">
                {page.hero_cta_primary_text || 'Get started free'} <Icons.ArrowRight size={15} className="group-hover:translate-x-1 transition-transform" />
              </Link>
              <Link to={page.hero_cta_secondary_link || '/listings'} className="inline-flex items-center gap-2 border border-white/20 text-white font-display font-bold px-5 py-3 rounded-md hover:bg-white/8 transition-all hover:-translate-y-0.5 active:translate-y-0 text-sm">
                {page.hero_cta_secondary_text || 'Browse storage'}
              </Link>
            </div>
          </div>
        </div>
          ...
          <div className="max-w-container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {hasMoreServices && (
              <div className="mt-10 text-center">
                <button onClick={loadMoreServices} className="inline-flex items-center gap-2 px-8 py-3 border border-[#1c1917] text-[#1c1917] font-display font-bold rounded-xl hover:bg-[#1c1917] hover:text-white transition-colors text-sm">
        ...
        <section className="py-24 bg-white border-y border-border">
          <div className="max-w-container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid lg:grid-cols-3 gap-8">
              {roleSteps.map((r) => (
                <Reveal key={r.id} delay={r.display_order * 100} direction="up">
                  <div className={`border ${r.border_color} rounded-xl p-6 h-full hover:shadow-lg hover:-translate-y-1 transition-all duration-300`}>
                    <div className={`inline-block text-xs font-bold uppercase tracking-widest px-3 py-1.5 rounded-full mb-5 ${r.badge_bg} ${r.badge_text}`}>{r.role}</div>
                    <ul className="space-y-3">
                      {Array.isArray(r.steps) && r.steps.map((step, j) => (
        ...
        <section className="py-24 bg-[#FAFAF9]">
          <div className="max-w-container mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid sm:grid-cols-3 gap-5 items-stretch">
              {pricingPlans.map((p) => (
                <Reveal key={p.id} delay={(p.display_order || 1) * 100} direction="scale">
                  <div className={`relative rounded-xl border p-7 h-full flex flex-col gap-6 transition-all duration-300 hover:-translate-y-2 hover:shadow-xl ${
                    p.highlight ? 'bg-[#1c1917] border-brick shadow-lg' : 'bg-white border-border hover:border-[#3a3a3a]'
                  }`}>
                    {p.highlight ? (
                      <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-brick text-white text-[10px] font-bold uppercase tracking-widest px-3 py-1 rounded-full whitespace-nowrap">
                        Most popular
                      </div>
                    ) : null}
                    <div>
                      <p className={`text-[10px] font-bold uppercase tracking-widest mb-3 ${p.highlight ? 'text-white/40' : 'text-[#71717a]'}`}>{p.name || 'Seller'}</p>
                      <p className={`font-mono font-bold text-5xl leading-none mb-1 ${p.highlight ? 'text-white' : 'text-[#1c1917]'}`}>{p.price || 'Free'}</p>
                    ...
                    <Link to={p.cta_link || '/register'} className={`mt-auto flex items-center justify-center gap-2 py-3 rounded-md text-sm font-display font-bold transition-all hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0 ${
                      p.highlight ? 'bg-brick hover:bg-brick-dark text-white' : 'bg-[#F4F4F5] hover:bg-[#E4E4E7] text-[#1c1917]'
                    }`}>
                      {p.cta_text || 'Get started'} <Icons.ArrowRight size={14} />
                    </Link>
                  </div>
                </Reveal>
              ))}

...
import { MapPin, Box, Truck, Shield, ChevronDown, ChevronUp, Star, Package, MessageSquare, ArrowRight, Check, Users, Building2 } from 'lucide-react';
...
const features = [
  { icon: MapPin, title: 'Storage near you', desc: 'Verified godowns and rooms across Kathmandu Valley. Filter by price, size and distance.' },
  { icon: Box, title: 'Track every item', desc: 'Know exactly what you have stored, where it is, and how much is left — updated live.' },
  { icon: Truck, title: 'Courier on demand', desc: 'Request a pickup from your dashboard. Couriers come to storage, you stay home.' },
  { icon: Shield, title: 'Hosts we verify', desc: 'Every storage host is checked by our team before going live. Your goods are safe.' },
  { icon: MessageSquare, title: 'Talk directly', desc: 'Message your host or courier without leaving the app. No WhatsApp needed.' },
  { icon: Package, title: 'Pay your way', desc: 'eSewa, Khalti or cash. Get a full invoice for every payment automatically.' },
];

const steps = [
  { num: '01', title: 'Find a space', desc: 'Search by your area. See pricing and host details before you book.' },
  { num: '02', title: 'Move your stock in', desc: 'Book the space, coordinate with the host, move in on your schedule.' },
  { num: '03', title: 'Manage from your phone', desc: 'Track stock, chat with your host and request pickups. All in one place.' },
  { num: '04', title: 'Scale without stress', desc: 'When orders grow, book more space. No lease, no deposit, no commitment.' },
];

const testimonials = [
  { name: 'Sachin', role: 'Instagram seller, Pokhara', text: 'Before this I kept stock in my bedroom. Now I have a proper space 5 minutes away and I can actually take bulk orders.', rating: 5 },
  { name: 'Nabin', role: 'Storage host, Pokhara', text: 'My spare room was doing nothing. Now it earns me Rs 8,000 a month and I barely have to do anything.', rating: 5 },
  { name: 'Lijas', role: 'Facebook seller, Pokhara', text: 'The inventory tracking alone is worth it. I always know exactly what I have in storage and when pickups are due.', rating: 5 },
];

const faqs = [
  { q: 'How does SamanBhandar work?', a: 'Sellers find nearby storage hosts, book a space, store their inventory, and schedule courier pickups with all from one dashboard.' },
  { q: 'How do I become a storage host?', a: 'Register as a host, list your unused room or garage, set your price, and start accepting bookings after our team verifies you.' },
  { q: 'Is my inventory safe?', a: 'All hosts are verified by our team before going live. You can also read reviews from other sellers before booking.' },
  { q: 'What payment methods are supported?', a: 'We support eSewa and Khalti for all payments. Cash is also accepted for in-person arrangements.' },
  { q: 'Can I cancel a booking?', a: 'Yes, you can cancel up to 24 hours before your scheduled move-in date for a full refund.' },
];
  ...
  return (
    <main className="bg-[#FAFAF9] overflow-x-hidden">
            ...
            <div>
              <div className={`transition-all duration-700 ease-out ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'}`}>
                <div className="inline-flex items-center gap-2 border border-border bg-white px-3 py-1.5 rounded-full text-xs font-semibold text-[#1c1917] mb-8 shadow-xs hover:shadow-md transition-shadow">
                  ...
                  Nepal's first micro-warehouse marketplace
                </div>
              </div>
              ...
              <div className={`transition-all duration-700 ease-out delay-100 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                <h1 className="font-display font-black text-5xl sm:text-6xl lg:text-7xl text-[#1c1917] leading-[1.0] tracking-tight mb-6">
                  ...
                  <span className="relative inline-block">
                    <span className="text-brick">Sell more.</span>
              ...
              <div className={`transition-all duration-700 ease-out delay-200 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                <p className="text-[#71717a] text-xl leading-relaxed mb-8 max-w-lg">
                  Stop stuffing your room with inventory. Find a safe storage space nearby, keep track of what's where, and handle pickups all from your phone.
                </p>
              </div>
              ...
              <div className={`transition-all duration-700 ease-out delay-300 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                <div className="flex flex-wrap gap-3 mb-10">
                  <Link to="/listings" className="group inline-flex items-center gap-2 bg-[#1c1917] text-white font-display font-bold px-7 py-4 rounded-xl hover:bg-brick focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#1c1917] transition-all text-sm shadow-sm hover:shadow-lg hover:-translate-y-0.5 active:translate-y-0">
                    Find storage near you <ArrowRight size={16} aria-hidden="true" className="group-hover:translate-x-1 transition-transform" />
                  </Link>
                  <Link to="/register?role=host" className="group inline-flex items-center gap-2 border border-border bg-white text-[#1c1917] font-display font-bold px-7 py-4 rounded-xl hover:border-[#1c1917] focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-[#1c1917] transition-all text-sm hover:shadow-md hover:-translate-y-0.5 active:translate-y-0">
                    List your space
                  </Link>
                </div>
              </div>
                ...
                <div className="flex items-center gap-8 pt-6 border-t border-border">
                  <div className="flex items-center gap-2.5 group cursor-default">
                    <div className="w-8 h-8 bg-white border border-border rounded-lg flex items-center justify-center group-hover:bg-[#1c1917] group-hover:border-[#1c1917] transition-all duration-300">
                      <Users size={14} className="text-[#1c1917] group-hover:text-white transition-colors duration-300" aria-hidden="true" />
                    </div>
                    <div>
                      <p className="font-display font-black text-lg text-[#1c1917] leading-none">{publicStats?.total_sellers || 0}+</p>
                      <p className="text-[10px] text-[#71717a] mt-0.5">Active sellers</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2.5 group cursor-default">
                    <div className="w-8 h-8 bg-white border border-border rounded-lg flex items-center justify-center group-hover:bg-[#1c1917] group-hover:border-[#1c1917] transition-all duration-300">
                      <Building2 size={14} className="text-[#1c1917] group-hover:text-white transition-colors duration-300" aria-hidden="true" />
                    </div>
                    <div>
                      <p className="font-display font-black text-lg text-[#1c1917] leading-none">{publicStats?.total_hosts || 0}+</p>
                      <p className="text-[10px] text-[#71717a] mt-0.5">Verified hosts</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2.5 group cursor-default">
                    <div className="w-8 h-8 bg-white border border-border rounded-lg flex items-center justify-center group-hover:bg-[#1c1917] group-hover:border-[#1c1917] transition-all duration-300">
                      <MapPin size={14} className="text-[#1c1917] group-hover:text-white transition-colors duration-300" aria-hidden="true" />
                    </div>
                    <div>
              ...
              <div className="relative">
                <div className="bg-white border border-border rounded-2xl p-6 shadow-sm hover:shadow-lg transition-shadow duration-300">
                  <div className="flex items-center justify-between mb-5">
                    <div>
                      <h2 className="font-display font-bold text-sm text-[#1c1917]">Storage Overview</h2>
                      <p className="text-[10px] text-[#71717a] mt-0.5">Live inventory dashboard</p>
                    </div>
                ...
                <div className="absolute -top-5 -right-5 bg-white border border-border rounded-xl px-4 py-3 shadow-sm animate-float-1 hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-2.5">
                    <div className="w-7 h-7 rounded-lg bg-brick-light flex items-center justify-center">
                      <Truck size={13} className="text-brick" aria-hidden="true" />
                    </div>
                    <div>
                      <p className="text-xs font-bold text-[#1c1917]">Pickup confirmed</p>
                      <p className="text-[10px] text-[#71717a]">Today at 2:00 PM</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="absolute -bottom-5 -left-5 bg-white border border-border rounded-xl px-4 py-3 shadow-sm animate-float-2 hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-bold text-[#1c1917]">Booking approved</p>
                      <p className="text-[10px] text-[#71717a]">By {publicStats?.recent_bookings?.[0]?.host_name || 'Host'}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <section className="py-24 bg-[#FAFAF9]">
        <div className="max-w-container mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="mb-16" direction="up">
            <p className="text-xs font-bold text-brick uppercase tracking-widest mb-3">How it works</p>
              ...
              <Reveal key={i} delay={i * 100} direction="scale">
                <div className="relative group h-full">
                  <div className="bg-white border border-border rounded-xl p-6 hover:-translate-y-2 hover:shadow-lg transition-all duration-300 ease-out h-full">
                    <p className="font-display font-black text-5xl text-border mb-5 group-hover:text-brick-light transition-colors duration-300">{step.num}</p>
        ...
        <div className="max-w-container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid lg:grid-cols-2 gap-16 items-center mb-16">
            <Reveal direction="left">
              <p className="text-xs font-bold text-brick uppercase tracking-widest mb-3">What you get</p>
            ...
            <Reveal delay={100} direction="right">
              <p className="text-[#71717a] text-lg leading-relaxed">
                Built specifically for Nepal's growing community of home-based sellers who need more than just a shelf in their bedroom.
              </p>
            </Reveal>
          </div>
              ...
              <Reveal key={i} delay={i * 60} direction="scale">
                <div className="group flex gap-4 p-5 border border-border rounded-xl hover:border-[#1c1917] hover:bg-chalk hover:shadow-md transition-all duration-300 h-full">
                  <div className="w-10 h-10 bg-chalk rounded-lg flex items-center justify-center shrink-0 group-hover:bg-[#1c1917] group-hover:scale-110 transition-all duration-300">
                    <f.icon size={17} className="text-[#1c1917] group-hover:text-white transition-colors duration-300" aria-hidden="true" />
                  </div>
                  <div>
      ...
      <section className="py-24 bg-[#FAFAF9]">
        <div className="max-w-container mx-auto px-4 sm:px-6 lg:px-8">
          <Reveal className="mb-16" direction="up">
            <p className="text-xs font-bold text-brick uppercase tracking-widest mb-3">Real sellers</p>
            <h2 className="font-display font-black text-4xl lg:text-5xl text-[#1c1917] tracking-tight leading-tight">
              What they're saying.
            </h2>
          </Reveal>
                ...
                <div className="bg-white border border-border rounded-xl p-6 h-full flex flex-col hover:border-[#3a3a3a] hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                  <div className="flex gap-0.5 mb-4" aria-label={`Rating: ${t.rating} out of 5 stars`}>
                    {Array.from({ length: t.rating }).map((_, j) => (
                      <Star key={j} size={13} className="fill-mustard text-mustard" aria-hidden="true" />
                    ))}
                  </div>
                  <p className="text-[#3a3a3a] text-sm leading-relaxed flex-1 mb-6">"{t.text}"</p>
                  <div className="flex items-center gap-3 pt-4 border-t border-border">
                    ...
                    <div>
                      <p className="font-display font-bold text-sm text-[#1c1917]">{t.name}</p>
      ...
      <section className="py-24 bg-white border-t border-border">
        <div className="max-w-2xl mx-auto px-4 sm:px-6">
          <Reveal className="mb-12" direction="up">
            <p className="text-xs font-bold text-brick uppercase tracking-widest mb-3">FAQ</p>
            <h2 className="font-display font-black text-4xl text-[#1c1917] tracking-tight leading-tight">Common questions.</h2>
          </Reveal>
              ...
              <Reveal key={i} delay={i * 50} direction="up">
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    <span className="font-display font-semibold text-sm text-[#1c1917]">{faq.q}</span>

...
import { LayoutDashboard, Building2, CalendarCheck, MessageSquare, DollarSign, UserCircle, BookOpen, LogOut, Warehouse, ChevronLeft, ChevronRight, Home } from 'lucide-react'
...
const NAV = [
  ...
  { to: '/host/docs', icon: BookOpen, label: 'Documentation' },
]
       ...
       <Link to="/" className={`flex items-center gap-3 px-4 h-14 border-b border-border hover:bg-chalk transition-colors ${collapsed ? 'justify-center' : ''}`}>
          ...
          {!collapsed && (
            <div>
              ...
              <p className="text-[#71717a] text-[10px] mt-0.5">Host Portal</p>
            </div>
          )}
        </Link>

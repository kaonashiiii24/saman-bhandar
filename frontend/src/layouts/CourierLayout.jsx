...
import { LayoutDashboard, Briefcase, Truck, MessageSquare, UserCircle,BookOpen, LogOut, Warehouse, ChevronLeft, ChevronRight, Home } from 'lucide-react'
import SidebarLink from '../components/common/SidebarLink'
import DashboardTopbar from '../components/common/DashboardTopbar'
import { useAuth } from '../hooks/useAuth'

const NAV = [
  { to: '/courier/dashboard', icon: LayoutDashboard, label: 'Dashboard' },
  { to: '/courier/jobs', icon: Briefcase, label: 'Available Jobs' },
  { to: '/courier/deliveries', icon: Truck, label: 'Deliveries' },
  { to: '/courier/chat', icon: MessageSquare, label: 'Chat' },
  { to: '/courier/profile', icon: UserCircle, label: 'Profile' },
  { to: '/courier/docs', icon: BookOpen, label: 'Documentation' },
]

const PAGE_TITLES = {
  '/courier/dashboard': 'Dashboard',
  '/courier/jobs': 'Available Jobs',
  '/courier/deliveries': 'Active Deliveries',
  '/courier/chat': 'Chat',
  '/courier/profile': 'Profile',
  '/courier/docs': 'Documentation'
}
...
export default function CourierLayout() {
  ...
  const title = PAGE_TITLES[location.pathname] || 'Courier'
        ...
        <Link to="/" className={`flex items-center gap-3 px-4 h-14 border-b border-border hover:bg-chalk transition-colors ${collapsed ? 'justify-center' : ''}`}>
          ...
          {!collapsed && (
            <div>
              ...
              <p className="text-[#71717a] text-[10px] mt-0.5">Courier Portal</p>
            </div>
          )}
        </Link>
